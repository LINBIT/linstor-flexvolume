...
type diskManager interface {
	MakeGlobalPDName(disk drbd) string
	// Attaches the disk to the kubelet's host machine.
	AttachDisk(disk drbdMounter) error
	// Detaches the disk from the kubelet's host machine.
	DetachDisk(disk drbdUnmounter, mntPath string) error
}

// utility to mount a disk based filesystem
func diskSetUp(manager diskManager, b drbdMounter, volPath string, mounter mount.Interface, fsGroup *int64) error {
	globalPDPath := manager.MakeGlobalPDName(*b.drbd)
	...
	if err = manager.AttachDisk(b); err != nil {
		glog.Errorf("failed to attach disk: %v", err)
		return err
	}
...
func diskTearDown(manager diskManager, c drbdUnmounter, volPath string, mounter mount.Interface) error {
	notMnt, err := mounter.IsLikelyNotMountPoint(volPath)
	if err != nil {
		glog.Errorf("cannot validate mountpoint %s", volPath)
		return err
